...
class Encoder(BasicModule):
    def __init__(self, vocab_pos_size=100, vocab_ner_size=100):
        super(Encoder, self).__init__()
        self.src_word_emb = nn.Embedding(config.vocab_size, config.emb_dim)
        self.emb_dim = config.emb_dim
        if config.use_pos:
            self.pos_embedding = nn.Embedding(vocab_pos_size, config.pos_dim)
            self.emb_dim+=config.pos_dim
        if config.use_ner:
            self.ner_embedding = nn.Embedding(vocab_ner_size, config.ner_dim)
            self.emb_dim+=config.ner_dim
        if config.use_idf:
            self.idf_embedding = nn.Embedding(config.num_idf_bins, config.idf_dim)
            self.emb_dim+=config.idf_dim
        self.lstm = nn.LSTM(self.emb_dim, config.hidden_dim, batch_first=True, bidirectional=True)
        self.fc = nn.Linear(config.hidden_dim * 2, config.hidden_dim * 2, bias=False)

        self.init_params()

    # seq_lens should be in descending order
    def forward(self, input, seq_lens, pos_inp=None, ner_inp=None, idf_inp=None):
        embedded = self.src_word_emb(input)
        if config.use_pos:
            pos_embedded = self.pos_embedding(pos_inp)
            embedded = torch.concat([embedded, pos_embedded], -1)
        if config.use_ner:
            ner_embedded = self.ner_embedding(ner_inp)
            embedded = torch.concat([embedded, ner_embedded], -1)
        if config.use_idf:
            idf_embedded = self.idf_embedding(idf_inp)
            embedded = torch.concat([embedded, idf_embedded], -1)
