...
from utils.utils import write_for_rouge_1, rouge_eval_1, rouge_log
...
class BeamSearch(object):
    def __init__(self, model_file_path):
        ...
        self._rouge_full_score_path = os.path.join(self._test_dir, 'full_score.txt')
        self._rouge_score_path = os.path.join(self._test_dir, 'score.txt')
        ...
        self.batcher = Batcher(self.vocab, config.decode_data_path, mode='decode',
                               batch_size=config.beam_size, single_pass=True, vocab_pos=self.vocab_pos, vocab_ner=self.vocab_ner)
        time.sleep(15)
    ...
    def run(self):
        ...
        while batch is not None:
            ...
            # print(batch.original_articles[0].decode())
            ...
            # raise Exception('ppp')
            write_for_rouge_1(original_abstract, decoded_words, counter,
        ...
        rouge_eval_1(self._rouge_ref_dir, self._rouge_dec_dir, self._rouge_full_score_path)
        df = pd.read_csv(self._rouge_full_score_path, sep=',')
        final_score = df.describe().loc['mean']
        final_score.to_csv(self._rouge_score_path, header=None, sep=':')
        print(open(self._rouge_score_path, 'r').read())
