    ...
    with open(out_file, 'wb') as writer:
        for idx, s in enumerate(story_fnames):
            ...
            if makevocab:
                ...
                tokens = [bytes(t.strip(), encoding='utf-8') for t in tokens]  # strip
...
def write_csv_to_bin(file, out_file, makevocab=False):
    """Reads the tokenized .story files corresponding to the urls listed in the url_file and writes them to a out_file."""
    print("Making bin file for csv file in %s..." % file)
    ...
    with open(out_file, 'wb') as writer:
        for idx in tqdm(range(num_data)):
            ...
            # if idx % 100 == 0:
            #     print("Writing row %i of %i; %.2f percent done" % (
            #     idx, num_data, float(idx) * 100.0 / float(num_data)))

            # Get the strings to write to .bin file
            original, summary = data['original'], data['summary']
            # original, summary, pos, ner = data['original'], data['summary'], data['pos'], data['ner']
            # pos_tag = []
            # ner_tag = []
            # posi = []
            # ori_sents = underthesea.sent_tokenize(original)
            # ori_tokens = []
            # for i, sent in enumerate(ori_sents):
                # pos = [tag[-1] for tag in underthesea.pos_tag(sent)]
                # ner = [tag[-1] for tag in underthesea.ner(sent)]
                # pos_tag.extend(pos)
                # ner_tag.extend(ner)
                # posi.extend([str(i)]*len(pos))
                # if makevocab:
                    # ori_tokens.extend(underthesea.word_tokenize(sent))
            if makevocab:
                ori_tokens = original.split()
                sum_tokens = summary.split()
                sum_tokens = [t for t in sum_tokens if
                              t not in [SENTENCE_START, SENTENCE_END]]  # remove these tags from vocab
                tokens = ori_tokens + sum_tokens
                tokens = [t.strip() for t in tokens]  # strip
                tokens = [t for t in tokens if t != ""]  # remove empty
                vocab_counter.update(tokens)
                # pos_tag = pos.split()
                # ner_tag = ner.split()
                # pos_counter.update(pos_tag)
                # ner_counter.update(ner_tag)
                # ner_counter.update(ner_tag)
                
            summary = f'{SENTENCE_START}{summary.strip()}{SENTENCE_END}'
            # Write to tf.Example
            tf_example = example_pb2.Example()
            tf_example.features.feature['article'].bytes_list.value.extend([bytes(original, encoding='utf-8')])
            # tf_example.features.feature['pos_tag'].bytes_list.value.extend([bytes(pos, encoding='utf-8')])
            # tf_example.features.feature['ner_tag'].bytes_list.value.extend([bytes(ner, encoding='utf-8')])
            # tf_example.features.feature['posi'].bytes_list.value.extend([bytes(' '.join(posi), encoding='utf-8')])
            tf_example.features.feature['abstract'].bytes_list.value.extend([bytes(summary, encoding='utf-8')])
            tf_example_str = tf_example.SerializeToString()
            str_len = len(tf_example_str)
            writer.write(struct.pack('q', str_len))
            writer.write(struct.pack('%ds' % str_len, tf_example_str))
    ...
    if makevocab:
        ...
        with open(os.path.join(finished_files_dir, "vocab"), 'w') as writer:
            for word, count in vocab_counter.most_common(VOCAB_SIZE):
                writer.write(word + ' ' + str(count) + '\n')
        print("Finished writing vocab file")
        # print( "Writing vocab pos tag file...")
        # with open(os.path.join(finished_files_dir, "vocab_pos"), 'w') as writer:
        #     for word, count in pos_counter.most_common(VOCAB_SIZE):
        #         writer.write(word + ' ' + str(count) + '\n')
        # print("Finished writing vocab ner tag file")
        # print( "Writing vocab ner tag file...")
        # with open(os.path.join(finished_files_dir, "vocab_ner"), 'w') as writer:
        #     for word, count in ner_counter.most_common(VOCAB_SIZE):
        #         writer.write(word + ' ' + str(count) + '\n')
        # print("Finished writing vocab ner tag file")
...
if __name__ == '__main__':
    ...
    write_csv_to_bin("/home/phongtt/pointer-generator/dataset/vietnews_clean_test.csv", os.path.join(finished_files_dir, "test.bin"))
    write_csv_to_bin("/home/phongtt/pointer-generator/dataset/vietnews_clean_val.csv", os.path.join(finished_files_dir, "val.bin"))
    write_csv_to_bin("/home/phongtt/pointer-generator/dataset/vietnews_clean_train.csv", os.path.join(finished_files_dir, "train.bin"), makevocab=True)
